...
export const metadata: Metadata = { title: "Clinical Diary" };
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
        <div>
          ...
          <h1 className="font-heading text-3xl font-extrabold tracking-[-0.035em]">Clinical Diary</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">View and manage appointments, patient arrivals and your working availability.</p>
        </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "Today’s Appointments", value: appointments.length, icon: CalendarDays, tone: "bg-primary-soft text-primary" },
        ...
        <article className="rounded-3xl border border-primary/12 bg-secondary p-6 text-secondary-foreground">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="font-heading text-lg font-extrabold text-white">Scheduling Safeguards</h2>
                <p className="mt-1 max-w-2xl text-xs leading-5 text-white/65">
                  Bookings are only offered during your available working hours.
                </p>
              </div>
            </div>
            <div className="flex flex-wrap gap-x-5 gap-y-2 text-xs font-semibold text-white/80 border-t border-white/10 pt-3 lg:border-t-0 lg:pt-0">
              <span>• Clinicians manage only their permitted diaries</span>
              <span>• Double-booking protection</span>
              <span>• Consistent booking availability</span>
            </div>
          </div>
        </article>
      </section>
